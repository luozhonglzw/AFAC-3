"""Hybrid: XGBoost for sl=1-2, Heuristic for sl=0/3+"""
import os
import numpy as np
import pandas as pd
...
def run():
    ...
    print(f"  Samples: {len(X_train)} (pos={sum(y_train)}, neg={len(y_train)-sum(y_train)})")
    ...
    def predict_hybrid(row, topk=10):
        ...
        if sl == 0:
            ...
            return predict_xgb(row, model, item_feats, user_feats, item_counts,
    ...
    cold_users = train_df[train_df["item_seq_raw"].apply(seq_len) == 0]
    if len(cold_users) > 0:
        val_cold = cold_users.tail(min(500, len(cold_users)))
        ndcg_scores = []
        for _, row in val_cold.iterrows():
            preds = predict_cold(row["uid"])
            ndcg_scores.append(ndcg_at_k(preds, row["target_iid"]))
        print(f"  sl=0: NDCG={np.mean(ndcg_scores):.4f} (n={len(val_cold)})")
